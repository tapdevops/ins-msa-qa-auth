 ...
 */
	 ...
	 */
		exports.finding_find = ( req, res ) => {
			...
			.then( data => {
				console.log(data);
				if ( data.length == 0 ) {
					var url = service_url + '/api/v2.2/finding';
					var args = {
						headers: {
							"Content-Type": "application/json", 
							"Authorization": req.headers.authorization
					...
					var dt = data[0];
					var start_date = Libraries.Helper.date_format( dt.TGL_MOBILE_SYNC.toString(), 'YYYYMMDDhhmmss' );
					...
					var urlFindingWithDate = service_url + '/api/v2.2/sync-mobile/finding/' + start_date + '/' + end_date;
		...
		exports.hs_block_find = ( req, res ) => {
			...
			.then( data => {
				if ( data.length == 0 ) {
					...
					var url = service_url + '/api/v2.2/sync-mobile/block/' + start_date + '/' + end_date;
	...
	/**
	 * Road
	 * Untuk mengambil data jalan di TM_ROAD.
	 * --------------------------------------------------------------------------
	 */
